...
%###############################################################################初始化程序变量###############################################################################
  ...
  if strcmp(process, 'yellow');
    ...
    vrchk(vrep, res);
       ...
       elseif strcmp(process, 'red')
           vrep.simxSetObjectOrientation(id,h.rgbdCasing,h.ref,[0 0 -120*pi/180], vrep.simx_opmode_oneshot);
       else strcmp(process, 'red')
           vrep.simxSetObjectOrientation(id,h.rgbdCasing,h.ref,[0 0 -120*pi/180], vrep.simx_opmode_oneshot);
       end
       res = vrep.simxSetIntegerSignal(id, 'handle_rgb_sensor', 1, vrep.simx_opmode_oneshot_wait);
       vrchk(vrep, res);  
       ...
       vrchk(vrep, res);
            ...
            vrchk(vrep, res);
             ...
             %获取并显示图像
              ...
              bw=im2bw(XYR);
                  fsm_sight='blue';
              elseif strcmp(fsm_sight,'blue')
              XYB=255*((RP_B-RP_G)>diff_B&(RP_B-RP_R)>diff_B); 
              %绘制黑白图
              figure(2)
              imshow(uint8(XYB));
              %转为二值图
             ...
             %寻找红色区域的最小外接矩形   
              ...
              cylinder_center=[arm_targetpos_armref(1)-0.03,arm_targetpos_armref(2)+0.02,arm_targetpos_armref(3)+0.05]
        ...
        elseif strcmp(fsm, 'capture')
           ...
           elseif strcmp(process, 'red') 
               chooseAngle = [62*pi/180, 75*pi/180, 10*pi/180, 0*pi/180, 0*pi/180];
           else strcmp(process, 'blue')
               chooseAngle = [62*pi/180, 75*pi/180, 10*pi/180, 0*pi/180, 0*pi/180];
           end
        ...
        elseif strcmp(fsm, 'noIK2')
            % Define each angle of the robot arm.0，  30，  52.4，  72.68,  90
           chooseAngle = [180*pi/180, 30*pi/180, 40*pi/180, 72.68*pi/180, 0*pi/180]; 
            % Apply the value to each articulation.
            for i = 1:5
                res = vrep.simxSetJointTargetPosition(id, h.armJoints(i), chooseAngle(i), vrep.simx_opmode_oneshot);
                vrchk(vrep, res, true);
       ...
       elseif strcmp(fsm, 'release')
        ...
        elseif strcmp(fsm, 'Rot100');
            % Rotate. 
            rotVel = angdiff( pi , youbotEuler(3));
...
%开始有限状态机
	...
	%结束状态机相关代码
        num=num+1;
        %fsm='rotate';
        if target==traj(end,:)&strcmp(fsm, 'finished')&strcmp(process, 'yellow')
            %break;
            fsm='rotate';
            process='red';
        elseif target==traj(end,:)&strcmp(fsm, 'finished')&strcmp(process, 'red')
            fsm='rotate';
            process='blue';
        else target==traj(end,:)&strcmp(fsm, 'finished')&strcmp(process, 'blue')
            fsm='rotate';
            process='yellow';          
            break
        end
