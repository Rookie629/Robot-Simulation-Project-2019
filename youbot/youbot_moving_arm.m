   ...
   % 初始化Hokuyo激光雷达获取所有相关句柄 
    ...
    vrchk(vrep, res, true);
        ...
        elseif strcmp(fsm,'identify')
           vrep.simxSetObjectOrientation(id,h.rgbdCasing,h.ref,[0 0 150*pi/180], vrep.simx_opmode_oneshot);    
            % 获取彩色相机像素信息
    % 通过信号量控制彩色相机开机
    res = vrep.simxSetIntegerSignal(id, 'handle_rgb_sensor', 1, vrep.simx_opmode_oneshot_wait);
    vrchk(vrep, res);
        ...
        elseif strcmp(fsm, 'capture')
           ...
           chooseAngle = [-95*pi/180, 15*pi/180, 130*pi/180, -45*pi/180, 0*pi/180]; 
